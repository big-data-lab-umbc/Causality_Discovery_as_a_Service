...
@task
def prepare(conn,username,password):
    ...
    try:
        conn.sudo('rm -rf causality-analytics')
    except:
        pass
    conn.run('git clone https://%s:%s@github.com/big-data-lab-umbc/causality-analytics.git'%(username, password))
    with conn.cd('/home/hadoop/causality-analytics/EnsembleCausality/Journal'):
        ...
        # conn.run('wget -v -O 5v_nonlinear_50M.csv -L https://umbc.box.com/shared/static/dq7ongvg3gpkieq7xd5n0n6zmv5xwfjm.csv')
        # conn.run('hadoop fs -put 5v_nonlinear_50M.csv')

@task
def start(conn, experiment_name):
    with conn.cd('/home/hadoop/causality-analytics/EnsembleCausality/Journal'):
        conn.run('spark-submit --master yarn \
            ...
            # --files file:///home/hadoop/causality-analytics/EnsembleCausality/Journal/5v_nonlinear_50M.csv \
            # two_phase_algorithm_data.py 3 5v_nonlinear_50M.csv 240 3 -v')
